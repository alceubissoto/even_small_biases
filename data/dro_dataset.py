...
class DRODataset(Dataset):
    ...
    def get_loader(self, train, reweight_groups, **kwargs):
        if not train: # Validation or testing
            ...
            shuffle = False
            sampler = None
        elif reweight_groups == 'group': # Training but not reweighting
            print('using reweighted loss based on group')
            ...
            shuffle = False
        elif reweight_groups == 'class':
            print('using reweighted loss based on class')
            group_weights = len(self)/self._y_counts
            weights = group_weights[self._y_array]

            # Replacement needs to be set to True, otherwise we'll run out of minority samples
            sampler = WeightedRandomSampler(weights, len(self), replacement=True)
            shuffle = False
        else:
            print('not using reweighted loss')
            shuffle = True
            sampler = None
